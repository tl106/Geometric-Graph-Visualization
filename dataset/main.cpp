...
void randpoint(double p[COL][ROW])
{
    ...
    {
        a[i] = rand() / (double)(RAND_MAX / 100) - 50; // -50~50
        a[i] = floor(a[i] * 100) / 100;
    }

    ofstream File;
    File.open("N.txt", std::ios::out | std::ios::trunc); // 只写；打开文件，清空内容
    if (!File.is_open())
    {
        return;
    }
...
void randedge(double p[COL][ROW])
{
    double b[5000][3];
    int edgenum = 0;
    int k = 0;
    ...
    ofstream Edge;
    Edge.open("P.txt", std::ios::out | std::ios::trunc); // 只写；打开文件，清空内容
    if (!Edge.is_open())
    {
        return;
    }
