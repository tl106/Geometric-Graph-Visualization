...
void CreateGraph(Graph* graph) {
    ...
    FILE* fp1 = NULL;
    if ((fp1 = fopen(PATH_NAME, "r")) == NULL) {  // 打开path.txt文件
...
void evaluate(double* xCoordinate, double* yCoordinate, double* zCoordinate, Graph* graph) {
    ...
    double avg_err = total_err / count_edge;
...
void display(void)
{
    ...
    for (int i = 0; i < 100; i++) {
        for (int j = 0; j < 100; j++) {
            graph1.matrix[i][j] = INFINITE;
        }
    }
    int first, second;
    float d;
    ifstream finp(PATH_NAME);
    for (int i = 0; i < 1221; i++) {
        finp >> first;
        finp >> second;
        finp >> d;
        graph1.matrix[first][second] = d;
    }
    finp.close();
    ...
    for (int i = 0; i < graph1.vexnum; i++) {
        for (int j = i; j < graph1.vexnum; j++) {
            if (graph1.matrix[i][j] != INFINITE) {
                glPushMatrix();
    ...
    for (int i = 0; i < 3 * graph1.vexnum; i = i + 3) {
