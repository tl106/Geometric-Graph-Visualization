...
void init(float **distance, int *vertex_desc, int *vertex_state, float **position, char *file_name)
{
...
{
    ofstream white;
    white.open("/Users/hanzhekaiscomputer/Desktop/output.txt"); // output文件
    for (int i = 0; i < VERTEX_NUM; i++)
    {
...
int main(int argc, char *argv[])
{
    ...
    ofstream file;
    file.open("/Users/hanzhekaiscomputer/Desktop/output_V1.csv", ios::app);
    ...
    for (int count = 1; count <= 100; count++) // 改
    {
        ...
        string str;
        ostringstream oss;
        oss << "/Users/hanzhekaiscomputer/Desktop/dataset3/P" << count << "-" << VERTEX_NUM << ".txt";
        str = oss.str();
        ...
        init(p2distance, vertex_desc, vertex_state, p2position, file_name);
        ...
        while (true)
        {
            ...
            if (err_edge - error_rate < 0.01)
            {
                ...
                file << fixed << setprecision(6) << file_name << "," << output_error << "," << output_edge << "," << output_distance << "," << error_rate << "," << output_error_by_distance << "," << time_span << endl;
                break;
            }
            err_edge = error_rate;
            count_iteration++;
